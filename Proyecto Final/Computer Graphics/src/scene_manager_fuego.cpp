...
std::vector<std::unique_ptr<scene>> scene_manager_fuego::sceneList;
int scene_manager_fuego::currentScene = -1;

void scene_manager_fuego::start(int argc, char* argv[], const std::string& name, int width, int height)
{
...
void scene_manager_fuego::initialize()
{
	...
	std::unique_ptr<scene> scene1(new scene_fuego);
...
void scene_manager_fuego::mainLoop()
{
	time::tick();
...
void scene_manager_fuego::idle()
{
	glutPostRedisplay();
}

void scene_manager_fuego::cleanup()
{
	sceneList.clear();
	currentScene = -1;
}

void scene_manager_fuego::resize(int width, int height)
{
	if (currentScene >= 0)
		sceneList.at(currentScene)->resize(width, height);
}

void scene_manager_fuego::normalKeysDown(unsigned char key, int x, int y)
{
	if (key == '+')
		next();
...
void scene_manager_fuego::normalKeysUp(unsigned char key, int x, int y)
{
	if (currentScene >= 0)
		sceneList.at(currentScene)->normalKeysUp(key);
}

void scene_manager_fuego::specialKeys(int key, int x, int y)
{
	if (currentScene >= 0)
		sceneList.at(currentScene)->specialKeys(key);
}

void scene_manager_fuego::passiveMotion(int x, int y)
{
	if (currentScene >= 0)
